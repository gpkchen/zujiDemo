...
-(void)example01{
    ...
    //广告数据请求
    _p_AppActiveInfo *param = [[_p_AppActiveInfo alloc] init];
    param.activeType = @"2";
    
    [[ZYHttpClient client] post:param showHud:NO success:^(NSURLSessionDataTask *task, ZYHttpResponse *responseObj) {

        NSLog(@"广告数据 = %@",responseObj);
        if (responseObj.success) {
            
            if (responseObj.data == NULL || responseObj.data == nil) {
                [self skipAction];
                return;
            }
            ...
            if (XH_IPHONEX) {
                if ([NSString isBlank:model.imgUrl]) {
                    [self skipAction];
                    return;
                }
            ...
            //缓存机制(仅对网络图片有效)
            //为广告展示效果更好,可设置为XHLaunchAdImageCacheInBackground,先缓存,下次显示
...
-(void)example02{
    ...
    //广告图片URLString/或本地图片名(.jpg/.gif请带上后缀)
    imageAdconfiguration.imageNameOrURLString = @"gif_shengji.gif";
    //设置GIF动图是否只循环播放一次(仅对动图设置有效)
    imageAdconfiguration.GIFImageCycleOnce = NO;
    //图片填充模式
    imageAdconfiguration.contentMode = UIViewContentModeScaleAspectFill;
...
-(void)xhLaunchAd:(XHLaunchAd *)launchAd customSkipView:(UIView *)customSkipView duration:(NSInteger)duration{
    ...
    //设置时间
    [button setTitle:[NSString stringWithFormat:@"%ld 跳过",(long)duration] forState:UIControlStateNormal];
}

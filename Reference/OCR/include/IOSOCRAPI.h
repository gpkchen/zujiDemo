...
NSString *TREC_GetVersionArray(void);// 获取版本信息
NSString *TREC_GetCopyrightInfoArray(void);//获取版权信息
NSString *TREC_GetTimeStatus(void);// 获取引擎测试时间
Gint TREC_StartUP(void);//引擎初始化
...
Gint TREC_OCR(void);// 引擎识别
Gint TREC_FreeImage(void);// 引擎识别(图片路径形式)
TCARD_TYPE TREC_GetCardType(void);// 引擎识别
NSString *TREC_GetOcrString(void);// 获取全部的信息
NSString *TREC_GetFieldString(TFIELDID  field);// 获取各个栏的识别信息
UIImage *TREC_GetFieldImage(TFIELDID  field);// 获取对应的图片流
NSString *TREC_GetJsonStringBuffer(void);// 获取识别信息json流数据
Gint  TREC_GetCardNumState(void);// 获取身份证证号状态
UIImage *TREC_GetHeadImage(void);// 获取人头像图片流
Gint TREC_SaveHeadImage(NSString *path);// 保存人头像到本地
Gint TREC_ClearUP(void);//引擎释放
...
Gint TBANK_OCR(void);// 银行卡识别
Gint TBANK_SaveImage(NSString *ImagePath);   // 银行卡保存图片
Gint TBANK_FreeImage(void);                      // 银行卡释放函数
UIImage *TBANK_GetSmallImage(void);             // 获取识别区域图片
